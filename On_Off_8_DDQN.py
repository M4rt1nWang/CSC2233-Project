...
def RL_Caching(M, N, num_of_video, num_of_client, cache_size, s_len, l_len, zipf_param):
    ...
    class cache_env:
        ...
        def LRU(self):
            if self.rq in self.state:
                self.state.remove(self.rq[0])
        ...
        for m in range(M):
            ...
            for i in range(N):
                ...
                for n in range(num_of_client):
                    ...
                    if cache[n].rq in cache[n].state or len(cache[n].state) < cache[n].cs:
                        cache[n].step(1)
                    else:
                        Train[n] = 1
                        rq = list(cache[n].rq)
