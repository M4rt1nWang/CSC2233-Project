...
def RL_Caching(M, N, num_of_video, num_of_client, cache_size, zipf_param, Algorithm):
    ...
    class cache_env:
        ...
        def step(self, replace, Algorithm = 'FIFO'):
            prev_state = bit(self.VN, self.N, self.request)
            count = 0
    ...
    X = tf.placeholder(shape=[num_of_client, num_of_video], dtype = tf.float32)
    W = tf.Variable(tf.random_uniform([num_of_video, action_state], 0, 0.01))
    ...
    with tf.Session() as sess:
        for m in range(M):
            ...
            for i in range(N):
                e = 1/(i/10+1)
                Qs = sess.run(Qpred,feed_dict={X:bit(cache.VN,cache.N,cache.request)})
                ...
                state = bit(cache.VN, cache.N, cache.request)
...
plt.legend(['FIFO', 'FIFO (XOR)', 'LFU', 'LFU (XOR)', 'LRU', 'LRU (XOR)', 'LRU (XOR + RL)'])
plt.savefig("On_Off_7_NoneXOR_OnlyRequest_1")
plt.clf()
...
plt.legend(['FIFO', 'FIFO (XOR)', 'LFU', 'LFU (XOR)', 'LRU', 'LRU (XOR)', 'LRU (XOR + RL)'])
plt.savefig("On_Off_7_NoneXOR_OnlyRequest_2")
plt.clf()
...
plt.legend(['FIFO', 'FIFO (XOR)', 'LFU', 'LFU (XOR)', 'LRU', 'LRU (XOR)', 'LRU (XOR + RL)'])
plt.savefig("On_Off_7_NoneXOR_OnlyRequest_3")
plt.clf()
...
plt.legend(['FIFO', 'FIFO (XOR)', 'LFU', 'LFU (XOR)', 'LRU', 'LRU (XOR)', 'LRU (XOR + RL)'])
plt.savefig("On_Off_7_NoneXOR_OnlyRequest_4")
plt.clf()
