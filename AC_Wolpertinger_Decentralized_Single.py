...
def RL_Caching(M, N, num_of_video, num_of_client, cache_size, s_len, l_len, K, zipf_param):
    ...
    class cache_env:
        def __init__(self, VN, cs, s_len, l_len, K, a=1):
            ...
            self.P /= sum(self.P)
            self.sub_P = np.array([1/(i**self.a) for i in range(1, self.VN+1)])
            self.sub_P /= sum(self.sub_P)
            np.random.shuffle(self.sub_P)
            rand_rho = np.random.rand()
            self.P = rand_rho * self.P + (1-rand_rho) * self.sub_P
            self.state = []
...
def FIFO_Caching(M, N, num_of_video, num_of_client, cache_size, zipf_param):
    ...
    class cache_env:
        def __init__(self, VN, N, cs, a=1):
            ...
            self.sub_P = np.array([np.array([1/(i**self.a) for i in range(1, self.VN+1)]) for _ in range(self.N)])
            self.sub_P /= np.array([np.sum(self.sub_P, 1)]).transpose()
            for k in range(self.N):
                np.random.shuffle(self.sub_P[k])
                rand_rho = np.random.rand()
                self.P[k] = rand_rho * self.P[k] + (1-rand_rho) * self.sub_P[k]
