        ...
        for m in range(M):
            ...
            for i in range(N):
                ...
                if np.random.rand(1) < e:
                    for n in range(cache.N):
                        if len(cache.state[n]) == 0:
                            a.append(np.random.randint(cache.VN))
                        else:
                            a.append(cache.state[n][np.random.randint(len(cache.state[n]))])
                else:
                    one_hot_state = bit(cache.VN, cache.N, cache.state)
                    for n in range(cache.N):
                        if sum(one_hot_state[n]) == 0:
                            a.append(np.random.randint(cache.VN))
                        else:
                            maxQ = max([x*y for x,y in zip(Qs[n], one_hot_state[n]) if y == 1])
                            a.append(np.where(Qs[n] == maxQ)[0][0])
                ...
                prev_state, count, reward, connection = cache.step(a)
                ...
                Qs = reward_batch + dis * maxQ * np.identity(action_state)[np.argmax(Qs1, 1)]
            ...
            for i in range(test_N):
                ...
                _, count, _, connection = cache.step(a)
